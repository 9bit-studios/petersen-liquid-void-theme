...
async function deployAllDatabases() {
  ...
  await createRelationships(databases);
  await setupAutomations();
  await createViews();
  
  console.log('✅ All databases deployed successfully!');
  return databases;
}
...
```javascript
async function createRelationships(databases) {
  console.log('Creating database relationships...');
  
  const [campaign, content, stl, design, dev] = databases;
...
```javascript
async function verifyDeployment(databases) {
  ...
  for (const test of tests) {
    try {
      const db = await notion.databases.retrieve({ database_id: test.id });
  ...
  console.log('🎉 Deployment complete! Your databases are ready to use.');
}
```
...
**Next Step**: Run `node deploy-all-databases.js` to create your portal!
