...
```javascript
// notion-setup-databases.js
const { Client } = require('@notionhq/client');
const dotenv = require('dotenv');
dotenv.config({ path: '/Users/pennyplatt/Documents/Oksana-creator-portal/.env.local' });
...
// (Continuing with all other databases from the implementation plan)
...
async function setupNotionPortal() {
  try {
    ...
    // Create Petersen Games databases
