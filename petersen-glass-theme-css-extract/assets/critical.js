...
function calculateHeaderGroupHeight(
   ...
   * There is a `ResizeObserver` and `MutationObserver` that kicks in post 
  hydration in header.js
   * Note: header-group uses display: contents, so we must observe all 
  children.
   * 
   * FOUNDATION INTEGRATION: While we calculate dynamic heights, foundation
   nav heights
   * (--foundation-nav-height: 44px, --foundation-nav-height-mobile: 56px) 
  provide
   * the baseline Apple HIG standards that components should target.
   */
  (() => {
    // Original header component (if exists)
  ...
  `${headerHeight}px`);
      document.body.style.setProperty('--header-group-height',
  `${headerGroupHeight}px`);
    }

    // Glass theme headers
    const glassTopHeader = document.querySelector('.glass-top-header');
    const glassFilterBar = document.querySelector('.glass-filter-bar-container');

    if (glassTopHeader) {
      const topHeaderHeight = glassTopHeader.offsetHeight || 80;

      document.documentElement.style.setProperty('--foundation-header-height', `${topHeaderHeight}px`);

      if (glassFilterBar) {
        const filterBarHeight = glassFilterBar.offsetHeight || 56;
        const totalHeight = topHeaderHeight + filterBarHeight;
        document.documentElement.style.setProperty('--foundation-filter-bar-height', `${filterBarHeight}px`);
