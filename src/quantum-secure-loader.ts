...
class QuantumSecureLoader {
  ...
  constructor() {
    ...
    this.config = {
      ...
      quantumSecurePath: resolve(this.oksanaRoot, 'quantum-secure')
    };
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  loader.initialize()
    .then((success) => {
      if (success) {
        console.log('✅ Quantum-Secure loader initialized successfully');
        process.exit(0);
      } else {
        console.log('❌ Quantum-Secure loader initialization failed');
        process.exit(1);
      }
    })
    .catch((error) => {
